...
namespace Clam.Repository.Film
{
    public class FilmRepository : Repository<ClamUserFilm>, IFilmRepository
    {
        ...
        public IEnumerable<FilmCategorySelection> GetAllFilmsForGenreSelection(Guid id, string userName)
        {
            var films = _context.ClamUserFilms.ToList();
            var category = _context.ClamUserFilmCategories.Find(id);
            var joinTable = _context.ClamUserFilmJoinCategories.ToList();

            var userProfile = _userManager.FindByNameAsync(userName);
            ...
            else
            {
                ...
                foreach (var item in model)
                {
                    ...
                    if (item.Status == true)
                    {
                        ...
                        // Recently Added List (Gets sorted outside loops)
                ...
                // Recently Added Listing
                displayModel.RecentlyAdded.OrderByDescending(x => x.DateAdded.Ticks).Take(restriction_Standard).ToImmutableList();
                displayModel.Latest.OrderByDescending(x => x.DateAdded.Ticks).Take(restriction_General).ToImmutableList();
