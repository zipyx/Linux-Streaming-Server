...
namespace Clam.Areas.EBooks.Controllers
{
    ...
    public class EBooksController : Controller
    {
        ...
        private readonly StreamBooksDataUpload _uploadFile;
        ...
        private readonly string[] _permittedExtentions = { ".pdf", ".jpg", ".png", ".jpeg" };
        ...
        public EBooksController(UserManager<ClamUserAccountRegister> userManager, SignInManager<ClamUserAccountRegister> signInManager, RoleManager<ClamRoles> roleManager,
            ClamUserAccountContext context, IMapper mapper, IConfiguration config, IWebHostEnvironment environment, 
            ILogger<EBooksController> logger, StreamBooksDataUpload uploadFile, UnitOfWork unitOfWork)
        {
            ...
            _targetFolderPath = config.GetValue<string>("AbsoluteFilePath-Book");
            _fileSizeLimit = config.GetValue<long>("BookSizeLimit");
        }
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var model = await _unitOfWork.EBooksControl.GetAllUserEBooks(getUserName);
            return View(model);
        }

        [Authorize(Policy = "Contributor-Create")]
        [HttpGet("upload")]
        public IActionResult PostUploadBook()
        {
        ...
        {
            await _unitOfWork.EBooksControl.RemoveEBook(id);
            return RedirectToAction(nameof(Index));
        }
        ...
        {
            var result = await _unitOfWork.EBooksControl.GetAllGenres();
            return View(result);
        }
        ...
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> CreateGenre(AreaUserBooksCategory model)
        {
            try
            {
                await _unitOfWork.EBooksControl.AddAsyncGenre(model);
        ...
        {
            await _unitOfWork.EBooksControl.RemoveGenre(id);
            _unitOfWork.Complete();
            return RedirectToAction(nameof(Category));
        }
        ...
        public async Task<IActionResult> GenreSelect(Guid id)
        {
            if (!User.Identity.IsAuthenticated)
            {
                return View("AccessDenied");
            }
            var books = await _unitOfWork.EBooksControl.GetAllEBooksForGenreSelection(id, User.Identity.Name);
            var genre = await _unitOfWork.EBooksControl.GetAsyncGenre(id);
        ...
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> GenreSelect(Guid id, List<BooksGenreSelection> model)
        {
            if (!ModelState.IsValid)
            {
                return View("Error");
            }
            await _unitOfWork.EBooksControl.UpdateAllEBooksGenreSelection(id, model);
            _unitOfWork.Complete();
            return RedirectToAction(nameof(Category));
        }
        ...
        {
            var category = await _unitOfWork.EBooksControl.GetAsyncGenre(id);
            ViewBag.CategoryName = category.CategoryName;
            ViewBag.CategoryId = category.CategoryId;
            var model = await _unitOfWork.EBooksControl.GetAsyncGenreWithEBooks(id);
            return View(model.OrderBy(x => x.BookTitle));
        }
        ...
        {
            var requestFile = _context.ClamUserBooks.SingleOrDefault(m => m.BookId == id);
            if (requestFile == null)
            {
                return null;
            }
